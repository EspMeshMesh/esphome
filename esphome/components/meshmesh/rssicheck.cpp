...
void RssiCheck::setup() {
    clearAverageWindow(0);
}
...
uint8_t RssiCheck::handleFrame(uint8_t *buf, uint16_t size, uint32_t from, int16_t rssi, MeshmeshComponent *parent) {
    uint8_t err = 1;
	switch(buf[0]) {
...
void RssiCheck::pushAverageData(int16_t remote, int16_t local) {
	...
	if(mAverageWindowIndex >= RSSICHECK_WINDOWS_SIZE) mAverageWindowIndex=0;
}
...
void RssiCheck::calcAverageRssi(int16_t *remote, int16_t *local) {
	...
	for(int i=0;i<RSSICHECK_WINDOWS_SIZE;i++) {
		if(mAverageWindowRemote[i] == 0 || mAverageWindowLocal[i] == 0) break;
...
void RssiSensor::newRssiData(uint32_t rssi) {
	publish_state(rssi);
}
#endif
