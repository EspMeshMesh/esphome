...
async def to_code(config):
    ...
    var = cg.Pvariable(config[CONF_ID], MeshmeshComponent.new(config[CONF_BAUD_RATE], config[CONF_TX_BUFFER_SIZE], config[CONF_RX_BUFFER_SIZE]))
    cg.add(var.setChannel(config[CONF_CHANNEL]))
    cg.add(var.setAesPassword(config[CONF_PASSWORD]))
    print(config)
    if CONF_HARDWARE_UART in config:
        cg.add(
            var.set_uart_selection(
                HARDWARE_UART_TO_UART_SELECTION["UART0"]
            )
        )
