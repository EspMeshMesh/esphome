    ...
    case DISCCMD_BEACONS_SEND_REQ:
        if(len == sizeof(CmdStart_t)-1 && !isRunning()) {
            ...
            if(mStart.mask == 0 || (Discovery::chipId() & mStart.mask) == mStart.filter) {
                ...
                ESP_LOGD(TAG, "Discovery::handle_frame beacon reply id %02lX mask %d filt %d slots %d delay %ld", Discovery::chipId() & mStart.mask, mStart.mask, mStart.filter, mStart.slotnum, mBeaconDelay);
            }
            err = 0;
        }
        break;
