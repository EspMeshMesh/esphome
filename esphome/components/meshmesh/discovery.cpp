...
void Discovery::process_beacon(uint32_t id, int16_t  rssi1, int16_t  rssi2) {
	uint8_t i;
    ESP_LOGD(TAG, "discovery_process_beacon id:%06X rssi1:%d rssi2:%d\n", id, rssi1, rssi2);
	...
	if(i < DISCOVERY_TABLE_SIZE) {
		...
		if(i == discovery_table_index) discovery_table_index++;
	}
}
    ...
    case DISCCMD_START_REQ:
        if(len == sizeof(CmdStart_t)-1) {
            ...
            if(mRunPhase == 0) {
                ...
                if(mStart.slotnum < 10) mStart.slotnum = 100;
                mStartTime = millis();
                mRunPhase = 1;
    ...
    case DISCCMD_BEACONS_SEND_REQ:
        if(len == sizeof(CmdStart_t)-1 && !isRunning()) {
            ...
            if(mStart.mask == 0 || (Discovery::chipId() & mStart.mask) == mStart.filter) {
                ...
                ESP_LOGD(TAG, "Discovery::handle_frame beacon reply id %02X mask %d filt %d slots %d delay %d", Discovery::chipId() & mStart.mask, mStart.mask, mStart.filter, mStart.slotnum, mBeaconDelay);
            }
            err = 0;
        }
        break;
