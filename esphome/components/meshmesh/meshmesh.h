...
public:
    typedef enum { SRC_SERIAL, SRC_BROADCAST, SRC_UNICAST, SRC_MULTIPATH, SRC_POLITEBRD, SRC_CONNPATH, SRC_FILTER } DataSrc;
...
#endif
public:
    MeshmeshComponent(int buad_rate, int tx_buffer, int rx_buffer);
    void pre_setup();
...
private:
    ...
    static void multipathRecvCb(void *arg, uint8_t *data, uint16_t size, uint32_t from, int16_t rssi, uint8_t *path, uint8_t pathSize);
