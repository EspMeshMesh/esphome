...
MeshmeshComponent *MeshmeshComponent::getInstance() {
	return singleton;
}
...
void MeshmeshComponent::setup() {
	...
	if(!mPreferencesObject.load(&mPreferences)) {
		...
		mPreferences.channel = 6;
		mPreferences.txPower = 82;
...
wifi_error:
	...
	if(mAesPassword.size()==0) {
		os_memcpy(aespassword, "1234567890ABCDEF", 16);
	} else {
		MD5Builder md5_builder{};
		md5_builder.begin();
		md5_builder.add(mAesPassword.c_str());
		md5_builder.calculate();
		md5_builder.getChars(aespassword);
	}
...
#ifdef ARDUINO_ARCH_ESP32
	ESP_LOGCONFIG(TAG, "Sys cip ID: %08X", Discovery::chipId());
#else
...
void MeshmeshComponent::loop() {
	if(mBaudRate > 0) {
		...
		//if(avail) ESP_LOGD(TAG, "MeshmeshComponent::loop available %d", avail);
		while(avail--) user_uart_recv_data((uint8_t)mHwSerial->read());
	}

	if(mUartTxBuffer.filledSpace() > 0) flushUartTxBuffer();
#ifdef ARDUINO_ARCH_ESP32
	packetbuf->loop();
#endif
#ifdef USE_POLITE_BROADCAST_PROTOCOL
    if(mPoliteBroadcast) mPoliteBroadcast->loop();
#endif
#ifdef USE_MULTIPATH_PROTOCOL
	...
	if(mDiscovery.isRunning()) mDiscovery.loop(this);
	// Execute rrsicheck if is running
	if(mRssiCheck.isRunning()) mRssiCheck.loop(this);
...
void MeshmeshComponent::uartSendData(const uint8_t* buff, uint16_t len) {
	if(!mUseSerial) return;

	uint16_t i;
	mUartTxBuffer.pushByte(0xFE);
	for(i=0; i<len; i++) {
		if(buff[i] == 0xEA || buff[i] == 0xEF || buff[i]==0xFE) mUartTxBuffer.pushByte(0xEA);
...
void MeshmeshComponent::broadCastSendData(const uint8_t* buff, uint16_t len) {
	if(broadcast) broadcast->send(buff, len);
}

void MeshmeshComponent::uniCastSendData(const uint8_t* buff, uint16_t len, uint32_t addr) {
	if(unicast) unicast->send(buff, len, addr, UNICAST_DEFAULT_PORT);
}

#ifdef USE_MULTIPATH_PROTOCOL
void MeshmeshComponent::multipathSendData(const uint8_t* buff, uint16_t len, uint32_t addr, uint8_t pathlen, uint8_t *path) {
	if(!multipath) return;
	...
	for(int i=0;i<pathlen;i++) pkt->setPathItem(uint32FromBuffer(path)+i*sizeof(uint32_t), i);
	pkt->setPayload(buff);
...
#ifdef USE_SENSOR
	if(findSensor(hash) != nullptr) return SensorEntity;
#endif
#ifdef USE_BINARY_SENSOR
	if(findBinarySensor(hash) != nullptr) return BinarySensorEntity;
#endif
#ifdef USE_SWITCH
	//if(findSwitch(hash) != nullptr) return SwitchEntity;
#endif
#ifdef USE_LIGHT
	if(findLightState(hash) != nullptr) return LightEntity;
#endif
	return AllEntities;
}
...
#ifdef MM_USE_SERIAL1
	Serial1.printf("F%d %d\n",  avail, mUartTxBuffer.filledSpace());
#endif
	if(avail>0) {
...
void MeshmeshComponent::commandReply(const uint8_t* buff, uint16_t len) {
    ...
    case SRC_POLITEBRD:
#ifdef USE_POLITE_BROADCAST_PROTOCOL
        if(mPoliteFromAddress != POLITE_DEST_BROADCAST) mPoliteBroadcast->send(buff, len, mPoliteFromAddress);
#endif
		break;
	case SRC_CONNPATH:
#ifdef USE_CONNECTED_PROTOCOL
...
void MeshmeshComponent::handleFrame(uint8_t *buf, uint16_t len, DataSrc src, uint32_t from) {
	...
	case CMD_FIRMWARE_REQ:
		if(len==1) {
			const char *firm = App.get_compilation_time().c_str();
			uint8_t rep[32];
			uint16_t lenstr = App.get_compilation_time().length();
			if(lenstr > 31) lenstr = 31;
			rep[0] = CMD_FIRMWARE_REP;
			os_strncpy((char *)rep+1, App.get_compilation_time().c_str(), 31);
	...
	case CMD_GET_ENTITY_STATE_REQ:
		if(len == 4) {
			...
			case LightEntity: {
				...
				if(state->current_values.get_state()==0) value = 0;
				else value = (uint16_t)(state->current_values.get_brightness() * 1024.0);
				value_type = 1;
#endif
				} break;
			case TextSensorEntity: {
	...
	case CMD_SET_ENTITY_STATE_REQ:
		if(len == 6) {
			...
			case SwitchEntity: {
				...
				if(state != nullptr) {
					if(value) state->turn_on(); else state->turn_off();
					valuefound = 1;
				}
#endif
				} break;
    ...
    case CMD_MULTIPATH_SEND: // 76 AAAAAAAA BB CCCCCCCC........DDDDDDDD AABBCCDDEE...ZZ
        ...
        if(len>5) {
			...
			if(len>6+pathlen*sizeof(uint32_t)) {
				...
				for(int i=0;i<pathlen;i++) pkt->setPathItem(uint32FromBuffer(buf+6)+i*sizeof(uint32_t), i);
	...
	case CMD_FLASH_GETMD5:
		if(mDelayedArgSize == 9) {
			...
			while(length>0) {
				...
				address += readsize;
				length -= readsize;
				if(erased) for(int i=0;i<128;i++) if(buff[i]!=0xFF) { erased = false; break; }
			}
			md5.calculate();
#endif
#ifdef ARDUINO_ARCH_ESP32
...
#ifdef ARDUINO_ARCH_ESP8266
			while(length>0) {
				if(ESP.flashEraseSector(address/FLASH_SECTOR_SIZE)) res++; else break;
...
void MeshmeshComponent::user_broadcast_recv(uint8_t *data, uint16_t size, uint8_t *from) {
	...
	if(size == 0 || data[0]==0x7F) return;
	os_memcpy(&mBroadcastFromAddress, from, 4);
	uint32_t *addr = (uint32_t *)from;
	ESP_LOGD(TAG, "MeshmeshComponent::user_broadcast_recv from %08X size %d cmd %02X", *addr, size, data[0]);
...
void MeshmeshComponent::unicastRecv(uint8_t *data, uint16_t size, uint32_t from, int16_t rssi) {
	...
	if(mRssiCheck.useRssiStatistics()) mRssiCheck.newRssiData(from, rssi);
}

void MeshmeshComponent::multipathRecvCb(void *arg, uint8_t *data, uint16_t size, uint32_t from, int16_t rssi, uint8_t *path, uint8_t pathSize) {
	((MeshmeshComponent *)arg)->multipathRecv(data, size, from, rssi, path, pathSize);
}

void MeshmeshComponent::multipathRecv(uint8_t *data, uint16_t size, uint32_t from, int16_t rssi, uint8_t *path, uint8_t pathSize) {
	os_memcpy(mRecvFromId, (uint8_t *)&from, 4);
	mRecvPathSize = pathSize;
	if(mRecvPathSize) os_memcpy(mRecvPath, path, mRecvPathSize*sizeof(uint32_t));
	mRssiHandle = rssi;
...
#ifdef USE_CONNECTED_PROTOCOL
	ESP_LOGD(TAG, "MeshmeshComponent::onConnectedPathNewClien %06X:%04X", from , handle);
...
void MeshmeshComponent::sendLog(int level, const char *tag, const char *payload) {
	if(mBaudRate == 0 && mPreferences.log_destination == 0) return;

	//uint16_t taglen = os_strlen(tag);
	uint16_t payloadlen = os_strlen(payload);
	...
	*buffer_ptr = CMD_LOGEVENT_REP;
	...
	if(payloadlen>0) os_memcpy(buffer_ptr, payload, payloadlen);
	if(mBaudRate>0) uartSendData(buffer, buffersize);
	if(mPreferences.log_destination == 1) { if (broadcast) broadcast->send(buffer, buffersize); }
	else if(mPreferences.log_destination > 1) { if(unicast) unicast->send(buffer, buffersize, mPreferences.log_destination, UNICAST_DEFAULT_PORT); }
