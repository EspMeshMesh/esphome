	...
	if(size > sizeof(MultiPathHeaderSt)) {
		...
		if(size >= wsize) {
			if(mRecvDups.checkDuplicateTable(header->sourceAddress, 0, header->seqno)) {
				ESP_LOGE(TAG, "MultiPath duplicated packet received from %06X with seq %d", header->sourceAddress, header->seqno);
				return;
			}
    ...
    if(status) {
        ...
        if(header != nullptr) {
			if((header->flags & MULTIPATH_FLAG_RETRANSMIT_MASK) < MULTIPATH_MAX_RETRANSMISSIONS) {
				...
				ESP_LOGE(TAG, "MultiPath::radioPacketSent transmission error for %06X via %06X path %d/%d after %d try", header->trargetAddress, pkt->target8211(), header->pathIndex, header->pathLength, header->flags & 0xF);
