...
void PoliteBroadcastProtocol::receiveRadioPacket(uint8_t *data, uint16_t size, uint8_t *fromptr, int16_t rssi) {
	...
	if(mState == StateIdle) {
		uint32_t now = millis();
		if(mTimeStamp0 != 0) {
			ESP_LOGD(TAG, "PoliteBroadcastProtocol::receiveRadioPacket StateIdle early request from:%06X", from);
			return;
		}
		...
		ESP_LOGD(TAG, "PoliteBroadcastProtocol::receiveRadioPacket StateIdle from:%06X seq:%d dst:%08X src:%06X", from, mOutPkt->politeHeader()->sequenceNum, mOutPkt->politeHeader()->destAddr, pkt->politeHeader()->sourceAddr);
		...
		if(head->sourceAddr == mOutPkt->politeHeader()->sourceAddr && head->sequenceNum == mOutPkt->politeHeader()->sequenceNum) {
			ESP_LOGD(TAG, "receiveRadioPacket StateWaitEnd from:%06X seq:%d dst:0x%08X src:0x%06X", from, mOutPkt->politeHeader()->sequenceNum, mOutPkt->politeHeader()->destAddr,  mOutPkt->politeHeader()->sourceAddr);
			if(mOutPkt->checkReceived(from)) {
				// I haave eared the same packet from enuough sorces I can givup.
				_setIdle();
			}
		} else {
			ESP_LOGD(TAG, "PoliteBroadcast: Ignored packet while busy! from:%06X %06X!=%06X %d!=%d", from, head->sourceAddr, mOutPkt->politeHeader()->sourceAddr, head->sequenceNum, mOutPkt->politeHeader()->sequenceNum);
		}
	}
}
...
void PoliteBroadcastProtocol::send(const uint8_t *data, uint16_t size, uint32_t target) {
	...
	// Prepare packet
	pkt->calcDelays();
	ESP_LOGD(TAG, "PoliteBroadcastProtocol::send seq:%d dst:%08X src:%06X", header->sequenceNum, header->destAddr, header->sourceAddr);
