...
public:
    ...
    uint16_t getHandle() const { return getHeader()->nodeHandle; }
private:
    uint32_t mTarget = 0;
};
...
class ConnectedPath {
public:
	ConnectedPath(MeshmeshComponent *meshmesh, PacketBuf *packetbuf): mMeshMesh(meshmesh), mPacketBuf(packetbuf), mRecvDups() { mPacketBuf->setConnectedPath(this); }
    void setup(void);
    void loop();
    uint8_t send(ConnectedPathPacket *pkt, bool forward, bool initHeader);
    ...
    void setReceiveCallback(ConnectedPathReceiveHandler recvCb, ConnectedPathDisconnectHandler discCb, void *arg, uint32_t from, uint16_t handle);
...
private:
    void connectionSetInvalid(uint8_t index) { if(index < CONNPATH_MAX_CONNECTIONS) connectionSetInvalid(mConnectsions+index); }
    ...
    uint8_t connectionGetFirstInvalid() {
        ...
        for(i=0;i<CONNPATH_MAX_CONNECTIONS;i++) 
            if(mConnectsions[i].sourceAddr == CONNPATH_INVALID_ADDRESS) break; 
        return i;
    }
private:
